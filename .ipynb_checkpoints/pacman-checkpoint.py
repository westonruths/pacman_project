...
def readCommand(argv):
    ...
    parser.add_option('-e', '--testghosts', dest='testghost',
                      help=default(
                          'the test ghost agent TYPE in the ghostAgents module to use'),
                      metavar='TYPE', default='RandomGhost')
    ...
    # Choose a test ghost agent
    ghostType = loadAgent(options.testghost, noKeyboard)
    args['testghosts'] = [ghostType(i + 1) for i in range(options.numGhosts)]
...
def runGames(layout, pacman, ghosts, testghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    import __main__
    ...
    for i in range(numGames):
        ...
        if beQuiet:
            ...
            gameDisplay = display
            rules.quiet = False
            game = rules.newGame(layout, pacman, testghosts,
                        gameDisplay, beQuiet, catchExceptions)           
